# codex-java-lsp-mcp Java-only V3 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `codex-java-lsp-mcp` 改造成一个 Java-only、结果可信、增量新鲜、AST 驱动、JDT 精确增强、token-aware 且可由真实 golden 持续验证的 Agent Code Intelligence 引擎。

**Architecture:** MCP 请求在进入 RepoRuntime 时固定 repo generation 和绝对 deadline；一个独立 RepoChangeCoordinator 维护新鲜度；一个 JavaIndex Worker actor 使用 Tree-sitter Java 拥有静态索引、coverage 和原子 snapshot；`java_impact` 的所有 provider 只产出标准 EvidenceSignal，随后经家族饱和排序与 byte/token 受限的 readPlan planner 输出。JDT LS 保留为按需 exact semantic gateway，而不是静态索引前提。
...
├── repo-generation.ts                   # 单调 generation clock
├── repo-change-coordinator.ts            # JDT-independent watcher + pending queue
├── repo-runtime-manager.ts               # runtime 生命周期与 JDT admission
├── repo-resolver.ts
├── alias-registry.ts
├── layout-probe.ts
├── search/
...
## Task 3：实现 JDT 五态生命周期、start singleflight 和事务清理
...
private async startTransactional(budget: DeadlineBudget): Promise<void> {
  if (!this.jdtlsBin) {
    throw new JavaIntelligenceError("JDT_BROKEN", "jdtls executable was not found");
  }
...
Return `state`, `started`, `pid` and `startingPid` alongside the existing cache/build/JDK/watcher fields. Remove every alternative `started` calculation based only on process/connection presence.
...
```bash
git add src/jdtls-session.ts src/jdtls-session.test.ts src/test-support
git commit -m "fix(jdtls): make startup transactional and lifecycle-safe"
```
...
- [ ] **Step 5: 写 FIFO 与 waiter timeout 测试**
...
- Produces:
  ...
  - runtime-ready barrier before the first cacheable request。
...
Expected: `package.json` and lockfile updated; Node engine remains >=22.
...
test("generation advances monotonically and dirty clear is compare-and-set", () => {
  ...
  assert.equal(clock.advance("java change"), 2);
  assert.equal(clock.markDirty("watcher failure"), 3);
  assert.deepEqual(clock.snapshot(), { value: 3, dirty: true });
  clock.clearDirty(2);
...
```ts
const roots = watchRoots(this.repoRoot, this.layout());
...
`createRuntime()` constructs in this order:
...
Register coordinator listeners before `start()` so first event cannot be lost. Store `runtime.ready = coordinator.start()` and await it in `contextFor()/withContext()` before handling the first request. Before each request creates `RequestContext`, call `await coordinator.flushNow()` so already-delivered events cannot remain hidden behind debounce.
...
```bash
npm run build
node --test dist/repo-generation.test.js dist/repo-change-coordinator.test.js dist/repo-runtime-manager.test.js
npm test
git add package.json package-lock.json src/repo-generation.ts src/repo-generation.test.ts src/repo-change-coordinator.ts src/repo-change-coordinator.test.ts src/repo-runtime-manager.ts src/file-watcher.ts
...
```ts
await entry.ready;                         // watcher has emitted ready
await entry.coordinator.flushNow();        // drain already-delivered debounced events
await entry.reconcileIfDirty();            // Task 11 singleflight; no-op while clean
await entry.coordinator.flushNow();        // include events delivered during reconcile
...
`reconcileIfDirty()` may be introduced as a no-op shell in this task and completed in Task 11, but its call position is fixed here. If the clock remains dirty after reconcile, the request proceeds with `coverage=DEGRADED`; it must not claim a complete negative result.
...
```ts
const cached = rgCache.get(key, request.generation);
if (cached) {
  return cached;
}
...
**Interfaces:**
- Consumes: Iteration B implementation、Task 0/Iteration A baseline artifacts、three real repo roots。
- Produces: mutation freshness evidence、edit-to-visible latency、cold quality/latency comparison and the Iteration B release decision。
...
npm run build
npm test
node --test --test-name-pattern="fast path cache invalidation|rename removes|delete removes|build change|last known good|stopped idle" "dist/**/*.test.js"
```
...
git commit -m "feat(index): add typed Java index worker protocol"
```
...
`extractType()` recursively handles its body with an enclosing type stack. Stable type IDs:

```text
type:<package>.<Outer>$<Inner>
```

For local/anonymous types without stable FQN:

```text
type-local:<relativePath>:<startLine>:<startColumn>
```

- [ ] **Step 6: 实现 method/field facts**

Method ID before type resolution:

```text
method:<ownerTypeId>#<name>/<arity>@<startLine>:<startColumn>
```

`signatureKey` includes normalized parameter type text and constructor flag. Do not require all types resolved before creating facts.
...
function emptyBundle(relativePath: string, simpleName: string, generation = 1): JavaFileBundle {
  ...
  const typeId = `type:demo.${simpleName}`;
...
function addMethod(bundle: JavaFileBundle, name: string): JavaMethodFacts {
  ...
  const methodId = `method:${owner.fqn}#${name}()`;
...
function addEdge(
  ...
  bundle.edges.push({
    ...base,
    edgeId: `edge:${edge.kind}:${edge.fromId}:${edge.toId}`,
...
type IndexJob = {
  ...
  kind: "REFRESH_FILES" | "FULL_SWEEP" | "RECONCILE" | "FLUSH";
  generation: number;
  files?: string[];
};
```
...
- watcher changes priority 0；
- full sweep priority 1；
...
- [ ] **Step 5: OPEN behavior**

```text
load snapshot
→ validate snapshot
→ state READY even if coverage partial
→ foreground refresh requested anchor files
→ schedule full sweep
```

First `java_impact` must not wait for whole sweep. It receives coverage PARTIAL and can use rg fallback.
...
```bash
npm run build
node --test dist/java-index/manifest.test.js dist/java-index/coverage.test.js dist/java-index/index-store.test.js
npm test
git add src/java-index src/repo-runtime-manager.ts
...
Use build stamp or a manually incremented extractor code version. Parser fixes must invalidate old AST cache.
...
```bash
npm run build
node --test dist/java-index/snapshot.test.js dist/java-index/build-fingerprint.test.js
npm test
...
- Produces: router uses async V2 facts; old V1 is removed after benchmark parity。
...
- Produces: index microbenchmarks、snapshot/freshness evidence、real-repo quality comparison and the Iteration C release decision。
...
- Modify: `src/semantic-gateway.ts` or current semantic owner
...
npm test
git add src/agent-router/reference-ranking.ts src/agent-router/reference-ranking.test.ts src/agent-router/providers/semantic-provider.ts src/semantic-gateway.ts
...
```bash
npm run build
node --test dist/java-index/mybatis-xml-extractor.test.js dist/agent-router/framework/mybatis.test.js
npm test
...
- [ ] **Step 6: Matrix runner**

`scripts` or `src/benchmark/matrix-runner.ts` accepts env roots and runs:
...
Outputs all JSON under commit-stamped directory and a summary markdown/JSON. It must preserve nonzero exit if any hard gate fails.
...
## Task 33：SemanticGateway same-key singleflight、complete-only cache 与简单故障退避
...
Add to `SemanticGateway` or a focused private helper in the same file:
...
```text
success                         reset to 0
JDT_NOT_READY/JDT_BROKEN        increment
JDT_SERVER_ERROR                increment
caller deadline/cancel          do not increment
retry delay                     min(30_000, 250 * 2^(n-1))
explicit java_restart           clears backoff
```

When backoff is active, semantic provider returns a typed degraded outcome immediately; it does not repeatedly respawn JDT on every request.

Do not build a circuit-breaker framework, half-open state machine or metrics registry.
...
| Admission | concurrency | two repos starting at cap | active never exceeds cap |
...
| Snapshot | persistence | save/load/version/corruption | atomic recovery, no partial state |
...
Task 3  fix(jdtls): transactional lifecycle
...
Task 21 feat(index): atomic snapshot
...
Task 33 perf(semantic): singleflight/cache/backoff
...
| M-03 缺并发/故障测试 | Task 2–7、9–12、21、33–34 | deterministic transport/watcher/fs fault suite |
