# codex-java-lsp-mcp Java-only 极致代码智能架构与优化战略 V3

> **文档状态：权威架构基线（Authoritative Architecture Baseline）**  
> **版本：V3.0**  
> **日期：2026-07-23**  
> **适用项目：`lkyprogramer/codex-java-lsp-mcp`**  
> **产品定位：小而精悍、Java-only、面向 AI Coding Agent 的仓库级代码智能引擎**  
...
- `codex-java-lsp-mcp-java-only-development-execution-plan-v3-2026-07-23.md`

旧方案不再作为并行路线输入。旧方案中的有效结论已吸收进本文件；与“小而精悍、Java-only、个人项目”冲突的内容已明确删除。
...
> **在一个 Java 仓库内，以尽可能低的延迟和 token 成本，向 Coding Agent 提供足够准确、足够新鲜、可解释且可直接执行的代码影响面与阅读计划。**

该目标可进一步拆成五个不可替代的核心能力：
...
| 并发模型 | 一个 JavaIndex Worker actor；不建设通用 worker pool 平台 |
...
└──────────────────────────────┬──────────────────────────────┘
                               │
                     RequestContext
        repoRoot + generation + DeadlineBudget + mode
                               │
┌──────────────────────────────▼──────────────────────────────┐
│                         RepoRuntime                         │
│                                                             │
...
│  CandidateProviders                                         │
│   ├─ AST structural                                         │
│   ├─ import/type/call graph                                  │
│   ├─ Spring/MyBatis/JPA adapters                             │
│   ├─ bounded rg fallback                                    │
│   └─ JDT exact semantic                                     │
│      ↓                                                      │
│  EvidenceNormalizer → FamilySaturatingRanker                │
│      ↓                                                      │
│  TokenAwareReadPlanPlanner                                  │
│      ↓                                                      │
...
每个 canonical repoRoot 对应一个 RepoRuntime，包含：
...
RepoRuntime 停止后，JDT 进程可关闭；长时间未使用的整个 context 必须从 Map 中回收，避免 SourceIndex 和 snapshot metadata 永久常驻。
...
export interface DeadlineBudget {
  ...
  race<T>(stage: string, operation: Promise<T>, capMs?: number): Promise<T>;
}
```
...
这些值是初始配置，最终以固定机器 benchmark 校准。任何子阶段只能消费剩余预算。
...
7. `start()` 只在 watcher 发出 `ready` 后完成；RepoRuntime 的第一次 cacheable 请求必须等待该 barrier。
8. 每次请求固定 generation 前先执行 `flushNow()`，把已经到达但仍在 debounce 窗口内的事件纳入当前 generation。
...
generation 是进程内单调序号，不要求跨进程持久。snapshot 中保存 `indexedGeneration` 和文件 fingerprint，用于判断缓存是否能加载。
...
- schema/extractor mismatch：直接丢弃重建；
- canonical root mismatch：拒绝加载；
- JSON/gzip corruption：删除损坏 snapshot，进入 rebuild；
- 进程在 rename 前崩溃：旧 snapshot 仍完整；
- 不写旧 JSONL migration；
- 旧 `source-index.*.jsonl` 和旧 edge snapshot 在 V2 首次启动时删除。
...
## 11.4 Active repo admission

RepoRuntimeManager 把 `STARTING` 和 `READY` 都计入 active slot。reservation 必须在任何 `await` 前同步写入，避免两个 repo 同时看到空位。

个人项目不建设复杂 lease system。采用：

- 一个全局 FIFO wait queue；
- entry reservation 状态；
- 最老 idle READY repo 可被停止；
- deadline 到期则失败；
- start fail/stop 后释放 reservation。
...
每个 entry 维护 shared backend promise、AbortController 和 waiter count。每个调用者以自己的 `DeadlineBudget` 等待 shared result，并在返回时生成自己的 `cacheHit/shared` 字段；短 deadline 调用者退出不会取消仍有其他 waiter 的 backend work。只有所有 waiter 都退出或 backend hard cap 到期时才发送 transport cancel。
...
完成条件：mutation stale rate=0，fast 模式可实时观察编辑。
...
| concurrent JDT oversubscribe | 0 |
...
| 通用 resource admission | 保留简单 active slots；实时 RSS 延后 |
...
12. warm-required 是否默认化由 Stage 5 数据决定；“继续不默认”是合法完成结果。
...
所有不直接服务这五项的扩张——多语言、图 UI、向量、RL、通用查询、企业级调度——都应拒绝。
