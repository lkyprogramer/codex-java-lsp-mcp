...
N4 出口 T3 四轴 **2/4**（token GO、p95 GO、RangeLineRecall FAIL、holdout rReadMust FAIL）。**2 < 3**，不得记 PARTIAL，不得开工 N5。escalate 走 0A.4 第 4 条 (b)：图 planner 无法在 ≥25% token 下降的同时保住 RangeLineRecall / holdout rReadMust。
...
- **N5 SKIP_FAIL**：`java_context` 与三臂 live A/B 不开工。N5 准入是 N4 ≥3/4 PARTIAL。
- **N6 SKIP_FAIL**：不删旧链、不合 `main`。N6 硬门含 §16.3 全过 + 第四仓 + leave-one-repo-out；N4 质量轴未过。

回滚点：分支 `codex/jin-main`，N4 证据 HEAD `90245a6`（discovery closeout）。不回滚已入库的 N0–N3 证据。
