...
- stdio 与后续 HTTP daemon 共用 canonical-root ownership；同一个 worktree 同一时刻只允许一个进程持有 JDT workspace、SourceIndex 和 cache。
...
| `java_shutdown` | 停止显式 selector 对应的 JDT LS 子进程，MCP server 保持存活；HTTP daemon 固定拒绝 `all=true`。 | 否 |
...
| `JDTLS_EXTRA_ARGS` | 追加传给 `jdtls` launcher 的参数，例如额外 `--jvm-arg=`；禁止包含 `-data`。 |
