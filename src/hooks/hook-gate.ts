...
try {
  ...
  if (!lsp.enabled || !looksJavaSemantic(prompt)) {
    ...
    writeAdvice(`JAVA_LSP_ADVISOR: 当前路径已启用 codex-java-lsp (${lsp.matchedBy})。先调用 java_status({repoRoot:"${lsp.effectiveRepoRoot}",start:false}) 校验 repoRoot，再优先用 java_impact 获取影响面。`);
  }
} catch {
  writeContinue();
}
