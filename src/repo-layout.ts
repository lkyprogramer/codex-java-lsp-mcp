...
export function repoCacheBase(env: NodeJS.ProcessEnv = process.env): string {
  const cacheBase = env.JAVA_LSP_CACHE_BASE?.trim();
  if (cacheBase) {
    return resolveConfiguredBase(cacheBase, "JAVA_LSP_CACHE_BASE", env);
  }
  const cacheRoot = env.JAVA_LSP_CACHE_ROOT?.trim();
  if (cacheRoot) {
