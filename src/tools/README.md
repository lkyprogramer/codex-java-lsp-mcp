...
本目录承载 public MCP tools 的薄 handler。公开工具面真源是 `src/mcp-server-factory.ts` 的 `PUBLIC_JAVA_TOOLS`（`java_status`、`java_impact`、`java_symbol`、`java_diagnostics`、`java_runtime`）。`java_impact` 仍是当前 compact 链。`java_context` handler 留在分支上，L1 FAIL 后不再注册。每个 handler 只负责入参解析、request budget 传播、调用共享服务和响应格式化。历史 v5 tools 面已删除。
...
- `java-context.ts` | JIN `java_context` handler（intent / 无锚点 / navigate）。L1 FAIL 后不在公开工具面。
